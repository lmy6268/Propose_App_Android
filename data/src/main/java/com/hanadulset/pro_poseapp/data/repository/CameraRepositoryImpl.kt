...
/**
 * This class provides the implementation of the camera-related business logic. It interacts with
 * the camera-related data sources and domain services to provide the necessary functionality for
 * the camera feature.
 */
@Singleton
class CameraRepositoryImpl
@Inject
constructor(@ApplicationContext private val applicationContext: Context) : CameraRepository {

    private val cameraDataSource by lazy { CameraDataSourceImpl(applicationContext) }
    ...
    private val analyticsManager by lazy { AnalyticsManager(applicationContext.contentResolver) }

    /**
     * Binds the camera to the given [LifecycleOwner], [Preview.SurfaceProvider], [aspectRatio],
     * [previewRotation], and [analyzer].
     *
     * @param lifecycleOwner the [LifecycleOwner] that controls the lifecycle of the camera
     * @param surfaceProvider the [Preview.SurfaceProvider] that provides the [Surface] for the
     * camera preview
     * @param aspectRatio the aspect ratio of the camera preview
     * @param previewRotation the rotation of the camera preview
     * @param analyzer the [ImageAnalysis.Analyzer] that is used to analyze images captured by the
     * camera
     * @return the [CameraStateModel] indicating the status of the camera binding
     */
    ...
    override suspend fun takePhoto(eventData: CaptureEventData): ProposeUri {
        return cameraDataSource.takePhoto().let { data ->
            val capturedImageBitmap =
                    data.use { ImageUtils.imageToBitmap(it.image!!, it.imageInfo.rotationDegrees) }
            ...
            analyticsManager.saveCapturedEvent(captureEventData = eventData, poseEstimationResult)
            resPhoto.toDomainWrapper()
        }
    }
     ...
     */
    override fun setZoomRatio(zoomLevel: Float) = cameraDataSource.setZoomLevel(zoomLevel)
    ...
    override fun sendCameraSound() {
        ...
        MediaActionSound().apply { play(MediaActionSound.SHUTTER_CLICK) }
    }

    /**
     * Sets the focus of the camera to the given [MeteringPoint] for the given duration in
     * milliseconds.
     *
    ...
    override fun unbindCameraResource() = cameraDataSource.unbindCameraResources()
     ...
     * @return the estimated pose as a list of [Triple]s, where each [Triple] represents the
     * position of a specific pose landmark
     */
    private suspend fun estimatePose(bitmap: Bitmap): List<Triple<Float, Float, Float>?> =
            suspendCoroutine { cont ->
                ...
                processTask
                        .addOnSuccessListener { resultPoseData ->
